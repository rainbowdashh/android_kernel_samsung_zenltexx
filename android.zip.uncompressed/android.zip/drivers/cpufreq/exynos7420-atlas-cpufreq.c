...
static void __init set_volt_table_CA57(void)
{
	...
	default :
		max_support_idx_CA57 = L3;		/* 2.2GHz */
	}
#else
...
int __init exynos_cpufreq_cluster1_init(struct exynos_dvfs_info *info)
{
	...
	/* booting frequency is 2.1GHz */
	info->boot_cpu_min_qos = exynos7420_freq_table_CA57[L4].frequency;
	info->boot_cpu_max_qos = exynos7420_freq_table_CA57[L4].frequency;
#ifdef CONFIG_SEC_PM
